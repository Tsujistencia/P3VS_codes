...
def update_NX(pitcher_name, year_list, stance_type, selected_nodes_list, num_classes_str, categories_list):
    ...
    actual_categories = categories_list
    if 'All' in categories_list:
        actual_categories = ['Out', 'StrikeOut', 'BaseHit', 'Walk', 'HomeRun']

    for category_item in actual_categories:
        ...
        for i in range(int(num_classes_str)):
            ...
            if os.path.exists(image_path):
                try:
                    ...
                    # 画像のスタイルを調整
                    category_images_div_content.append(html.Img(
                        src=f'data:image/png;base64,{encoded_img}',
                        style={'height': '250px', 'width': 'auto', 'objectFit': 'contain', 'margin':'10px'}
                    ))
                except Exception as e:
                    category_images_div_content.append(html.Div(f"Error loading image: {filename}", style={'margin':'10px'}))
            else:
                # 画像が見つからない場合も、レイアウトが崩れないようにプレースホルダーを表示
                category_images_div_content.append(html.Div(f"Image not found: {filename}", 
                                                      style={'height': '250px', 'width': '250px', 'margin':'10px', 'border': '1px solid lightgrey', 'display': 'flex', 'alignItems': 'center', 'justifyContent': 'center'}))

        if category_images_div_content:
            # カテゴリーごとにDivを作成し、タイトルと画像リストを含める
            category_html_block = html.Div([
                html.H4(f"Category: {category_item}", style={'textAlign': 'left', 'paddingLeft': '20px'}),
                html.Div(category_images_div_content, style={
                    'display': 'flex', 
                    'flexDirection': 'row', 
                    'flexWrap':'wrap', 
                    'gap': '10px',
...
def handle_interactions(start_clicks, reset_clicks, tapped_node_data, nodesize_slider_value,
    ...
    if triggered_element_id == 'reset-button':
        ...
        # Note: We are using the globally tracked sequence inside update_elements
        ...
        # We only update the sequence in the store, not all elements, to avoid flicker
        ...
        if 'All' in categories_to_process:
             # If 'All' is selected, process all major categories
             categories_to_process = ['Out', 'StrikeOut', 'BaseHit', 'Walk', 'HomeRun']

        for cat_item in categories_to_process: 
            # When calling select_extraction_data, pass only the specific category
            # The 'All' logic is now handled before this loop.
        ...
        # The update_NX function will generate the final HTML layout
