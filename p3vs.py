...
import pandas as pd
from dash import dcc          # <- ここでエラーが出る可能性 
from dash import html         # <- ここでエラーが出る可能性 
from dash import dash_table   # <- ここでエラーが出る可能性 
...
#image_directory = '/content/Player_NX' #相対パスにする
...
def handle_buttons(start_clicks, reset_clicks, tapped_node, nodesize_unit,
                   pitcher_name, year, stance_type, num_classes, Category, selected_nodes, elements):
...
def update_elements(tapped_node, elements, selected_nodes): #last_tapped_node=None):
...
#ChatGPT様様
def edge_merge(data):
  # キーごとに整数値を合計するための辞書を作成
  result = {}

  for key, value in data:
      normalized_key = tuple(sorted(key))  # キーの順序を無視する
...
def create_NX(clustered_data, pitcher_name, year, stance_type, selected_zones, num_classes, category, nodesize_unit):
    ...
    for i in range(0, int(num_classes)):
        ...
        # エッジ集計処理（省略）… edge_data00 にまとめている前提

        # G.nodes の順序に合わせて nodesize を作る
        nodesize = []
        for node_id in G.nodes:
            if node_id in allowed_nodes:
                cnt = node00.get(node_id, 1)
                # ← 以前の「50 * cnt」を「nodesize_unit * cnt」に置き換え
                nodesize.append(nodesize_unit * cnt)
            else:
                nodesize.append(nodesize_unit)  # 念のため、それ以外も最低 nodesize_unit
       
        # エッジ作成（カテゴリ分けして G.add_edge…）も従来どおり
...
def update_NX(pitcher_name, year, stance_type, selected_zones, num_classes, categories):
    ...
    for category in categories:  
        ...
        category_html = html.Div([
            html.H4(f"Category: {category}"),  # カテゴリー名を表示
            html.Div(category_images, style={'display': 'flex', 'flex-direction': 'row', 'gap': '10px','justify-content':'center'})  # 画像を横に並べて表示
        ])
...
def update_NX2(pitcher_name, year, stance_type, selected_zones,num_classes, category):
    images = []
    for i in range(0, int(num_classes)):
        filename = f"{pitcher_name}_{year}_{stance_type}_{category}_{selected_zones}_{num_classes}_{i}{image_extension}"
        image_path = os.path.join(image_directory, filename)
        ...
        if os.path.exists(image_path):
            with open(image_path, 'rb') as img_file:
                img = base64.b64encode(img_file.read()).decode('utf-8')
                # Display the image
                images.append(html.Img(src=f'data:image/png;base64,{img}', style={'height': '20%', 'width': '20%'}))
        else:
            # Display a message if the file does not exist
            images.append(html.Div(f"Image not found: {filename}"))
