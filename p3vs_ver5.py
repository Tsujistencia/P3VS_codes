...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.H3("Minimum Edge Weight"),
            dcc.Slider(
                id='min_edge_weight_slider',
                min=0, max=20, step=1, value=0,  # 必要に応じてmin, max, stepを調整
                marks={i: str(i) for i in range(0, 21, 5)},
                tooltip={"placement": "bottom", "always_visible": True},
                included=False
            ),
        ], style={'width': '30%', 'padding': '15px', 'display': 'inline-block', 'verticalAlign': 'top'}),
        ...
        ], style={'width': '35%', 'padding': '15px', 'display': 'inline-block', 'verticalAlign': 'top'})

    ], style={'display': 'flex', 'flexDirection': 'row', 'justifyContent': 'center'}),

    # ★ 変更: image-display の幅を100%に設定
...
def create_NX(clustered_data_df, pitcher_name, year_list, stance_type, selected_nodes_list, num_classes_str, category_item, nodesize_unit, min_edge_weight):
    ...
    for i in range(int(num_classes_str)):
        ...
        # エッジの重みが min_edge_weight 以上の場合のみエッジを追加
        filtered_edges = []
        for k_tuple, v_weight in merged_edge_counts.items():
            ...
            if not (k_tuple[0] in allowed_nodes and k_tuple[1] in allowed_nodes):
                continue
            if v_weight >= min_edge_weight:  # ここでフィルタリング
                filtered_edges.append((k_tuple, v_weight))

        # フィルタリングされたエッジを追加
        for k_tuple, v_weight in filtered_edges:
...
def update_NX(pitcher_name, year_list, stance_type, selected_nodes_list, num_classes_str, categories_list):
    ...
    out_column = _generate_column_content('HomeRun')
...
@app.callback(
    ...
    [Input('Start-button', 'n_clicks'),
     ...
     Input('min_edge_weight_slider', 'value')],  # ★ 追加: 新しいスライダーの入力
    [State('P-name', 'value'),
     State('Season', 'value'),
     State('Stype', 'value'),
     State('numc', 'value'),
...
def handle_interactions(start_clicks, reset_clicks, tapped_node_data, nodesize_slider_value,
                        min_edge_weight_value,  # ★ 追加: スライダーの値を受け取る
    ...
    # min_edge_weight_slider の値が変更されたときも描画を更新するように変更
    elif triggered_element_id == 'Start-button' or triggered_element_id == 'nodesize_slider' or triggered_element_id == 'min_edge_weight_slider':
        ...
        for cat_item in categories_to_process: 
            ...
            # min_edge_weight_value を create_NX に渡す
            create_NX(kmedoids_results_df, pitcher_name_state, season_state, stance_type_state, 
                      current_selected_nodes_from_store, num_classes_state, cat_item, 
                      nodesize_slider_value, min_edge_weight_value) # ここで新しい引数を渡す
