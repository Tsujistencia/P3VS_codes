...
# ======================================================================
# Elbow Method Helpers
# ======================================================================
def save_elbow_plot_kmedoids(
    D: np.ndarray,
    k_values: Sequence[int] = tuple(range(2, 11)),
    output_png: str = "elbow_kmedoids.png",
    random_state: int = 0,
    init: str = 'k-medoids++',
    verbose: bool = False,
):
    """
    距離行列 D（shape: (n_samples, n_samples)）を用いた K-Medoids による Elbow法プロットをPNG保存。

    Distortion は以下で近似:
        平均最小距離 = mean( min_j D[i, medoids[j]] )
    （クラスタ代表 medoid への最短距離の平均）
    ...
    Returns
    -------
    dict: {'k_values': [...], 'distortions': [...], 'png_path': str}
    """
    D = np.asarray(D)
    if D.ndim != 2 or D.shape[0] != D.shape[1]:
        raise ValueError("D は (n_samples, n_samples) の正方距離行列である必要があります。")

    distortions: List[float] = []
    n = D.shape[0]
    for k in k_values:
        if verbose:
            print(f"KMedoids Elbow: k={k} 計算中...")
        # metric='precomputed' で距離行列を直接渡す
        clusterer = KMedoids(n_clusters=k, random_state=random_state, init=init, metric='precomputed')
        clusterer.fit(D)
        medoid_idxs = clusterer.medoid_indices_
        # 各サンプル i から最も近い medoid までの距離を計算
        min_to_medoid = np.min(D[:, medoid_idxs], axis=1)
        # 平均最小距離（Distortionのproxy）
        distortions.append(float(np.sum(min_to_medoid) / n))

    # プロット
    plt.figure(figsize=(8, 5), dpi=150)
    plt.plot(list(k_values), distortions, 'bx-', linewidth=1.5, markersize=6)
    plt.title('Elbow Method (K-Medoids, precomputed distances)')
    plt.xlabel('Number of clusters (k)')
    plt.ylabel('Mean min distance to medoids')
    ...
    if verbose:
        print(f"K-Medoids Elbowプロットを保存しました: {output_png}")

    return {'k_values': list(k_values), 'distortions': distortions, 'png_path': output_png}
...
app.layout = html.Div([
    # ヘッダー
    html.Div([
        html.H1("⚾ MLB Pitcher's Pitching Patterns Visualization System ⚾", 
                style={
                    'textAlign': 'center', 
...
# Elbow plot generator for arbitrary dataframe (used when cache is reused)
def ensure_elbow_plot_for_df(p_name: str, df: pd.DataFrame,
                             w_traj: float, w_speed: float, w_pitch: float,
                             pitch_coords: dict) -> Optional[str]:
    """Ensure elbow plot exists in assets; generate if missing.

    Returns assets URL string or None if generation not possible.
    """
    try:
        output_path = ASSETS_DIRECTORY / f"{p_name}_{w_traj}_{w_speed}_{w_pitch}_Elbow.png"
        # ファイルが既に存在すれば即座に返却（高速化）
        if output_path.exists():
            logger.info(f'Elbow plot already exists: {output_path.name}')
            return f"/assets/{output_path.name}"

        if df is None or df.empty or len(df) < 2:
            logger.info('Not enough data to generate elbow plot')
            return None

        # Compute distance matrix like in perform_clustering
        trajectories = df.to_dict('records')
        n = len(trajectories)
        dist_traj = np.zeros((n, n))
        dist_speed = np.zeros((n, n))
        dist_pitch = np.zeros((n, n))
        ...
        logger.info('Elbow plot generated at %s', output_path)
        return f"/assets/{output_path.name}"
    except Exception:
        logger.exception('Failed generating elbow plot outside clustering path')
        return None
